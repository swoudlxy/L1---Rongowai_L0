...
rx_heading_pvt = double(ncread(L0_filename,'/geometry/receiver/rx_attitude_yaw_deg'));
...
rx_heading_pvt = rx_heading_pvt(index1);
...
rx_heading_pvt = rx_heading_pvt(index0_b:index0_e);
...
if ~isempty(index0)
    ...
    for l = 1:L
        ...
        rx_heading_pvt(index0_1) = mean([rx_heading_pvt(index0_1-1) rx_heading_pvt(index0_1+1)]);
...
if diff > 0
    ...
    rx_heading_pvt = rx_heading_pvt(offset_idx:end);
...
rx_heading = interp1(pvt_utc,rx_heading_pvt,ddm_utc,'linear','extrap');
rx_attitude = [rx_roll rx_pitch rx_heading];
...
L1_postCal.geospatial_lat_min = '-48.034';      % lat/lon correct to without N/E - 22 July 2023
L1_postCal.geospatial_lat_max = '-34.374';
L1_postCal.geospatial_lon_min = '165.319';
L1_postCal.geospatial_lon_max = '179.767';
...
L1_postCal.ac_heading_pvt = rx_heading_pvt;

L1_postCal.ac_roll = rx_attitude(:,1);
L1_postCal.ac_pitch = rx_attitude(:,2);
L1_postCal.ac_heading = rx_attitude(:,3);
...
L1a_xpol_calibration_flag = zeros(J,I)+invalid;
    ...
    for j = 1:J/2
        ...
        if ~isnan(tx_pos_x(j,i))
            ...
            if LOS_flag1 == 1
                ...
                L1a_xpol_calibration_flag(j,i) = L1a_xpol_calibration_flag_copol1;
                L1a_xpol_calibration_flag(j+J/2,i) = L1a_xpol_confidence_flag_xpol1;
...
L1_postCal.L1a_xpol_calibration_flag = L1a_xpol_calibration_flag;

% Part 3A ends

% Part 3B: Peak and theoretical SP index, zenith code phase 
% initialise variables
...
% first-scale-factor corrected raw counts
% new step below due to reordering
% 22 July 2023
ddm_power_counts = zeros(M,N,J,I)+nan;
...
for j = 1:J
    for i = 1:I
        ...
        ddm_power_counts(:,:,j,i) = first_scale_factor1*raw_counts1;
...
for i = 1:I
    ...
    for j = 1:J/2
        ...
        if ~isnan(tx_pos_x(j,i)) && ~isnan(sum(counts_LHCP1,'all'))
            ...
            peak_delay_row(j,i) = peak_delay_row1;            
            peak_doppler_col(j,i) = peak_doppler_col1;
...
% save variables
L1_postCal.brcs_ddm_peak_bin_delay_row = peak_delay_row-1;  % correct to 0-based index
L1_postCal.brcs_ddm_peak_bin_dopp_col = peak_doppler_col-1;

L1_postCal.brcs_ddm_sp_bin_delay_row = sp_delay_row;        % SP-idx already 0-based
...
% Part 4B: L1a calibration
% this part converts from raw counts to signal power in watts and complete
% L1a calibration
% significant changes to the L1a calibration curve and process 
% in this section - 22 July 2023
...
% set L1a power calibration flag
L1a_power_calibration_flag_LHCP = zeros(J/2,I);
L1a_power_calibration_flag_RHCP = zeros(J/2,I);

std_dev_rf2_db = repmat(mag2db(std_dev_rf2'),[10,1]);
std_dev_rf3_db = repmat(mag2db(std_dev_rf3'),[10,1]);
...
L1a_power_calibration_flag = [L1a_power_calibration_flag_LHCP; ...
    L1a_power_calibration_flag_RHCP];

% save outputs to L1 structure
L1_postCal.L1a_power_calibration_flag = L1a_power_calibration_flag;
...
for i = 1:I
    for j = 1:J
        ...
        rx_heading1 = rad2deg(rx_heading(i));
        ...
        % flag 8 and 11 - typo 14 July
        noise_floor1 = noise_floor(j,i);

        if i > 1
            noise_floor2 = noise_floor(j,i-1);
            diff1 = (noise_floor1-noise_floor2)/noise_floor1;
            diff2 = pow2db(noise_floor1)-pow2db(noise_floor2);

            if abs(diff1) > 0.1
                quality_flag1_1(8) = 1;     % typo 14 July
            end

            if abs(diff2) > 0.24
                quality_flag1_1(11) = 1;    % typo 14 July
            end
        end
        ...
        if dist_to_coast1>-5 && dist_to_coast1<0
        ...
        signal_code_phase1 = delay_correction(delay_center_chips(j,i),1023);
        ...
        if (sp_delay_row1<14)||(sp_delay_row1>35)   % typo: 14 July
            quality_flag1_1(14) = 1;
        end

        if (sp_dopp_col1<1)||(sp_dopp_col1>4)       % typo: 14 July
...
L1_postCal.quality_flags1 = quality_flags1;     % missing due to conflicts