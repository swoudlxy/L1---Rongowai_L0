% this function calibrates from raw DDM counts to DDM power in watts

function ddm_power_watts = L1a_counts2watts(raw_counts,ANZ_port,ddm_counts_cal_db,ddm_power_cal_dbm,std_dev)

binning_thres_db = [50.5 49.6 50.4];
cable_loss_db = [1.8051,0.6600,0.5840];

% select approiate calibration constants based on the input ANZ port channel
switch ANZ_port

    case 1
        ddm_counts_db_ch = ddm_counts_cal_db(1,:);
        ddm_power_dbm_ch = ddm_power_cal_dbm(1,:);
        
        std_dev_ch = std_dev(1);

        binning_thres_db_ch = binning_thres_db(1);
        cable_loss_db_ch = cable_loss_db(1);
        
    case 2
        ddm_counts_db_ch = ddm_counts_cal_db(2,:);
        ddm_power_dbm_ch = ddm_power_cal_dbm(2,:);

        std_dev_ch = std_dev(2);

        binning_thres_db_ch = binning_thres_db(2);
        cable_loss_db_ch = cable_loss_db(2);        
            
    case 3
        ddm_counts_db_ch = ddm_counts_cal_db(3,:);
        ddm_power_dbm_ch = ddm_power_cal_dbm(3,:);

        std_dev_ch = std_dev(3);

        binning_thres_db_ch = binning_thres_db(3);
        cable_loss_db_ch = cable_loss_db(3);

end

raw_counts_db = pow2db(raw_counts);
std_dev_db_ch = mag2db(std_dev_ch);

ddm_power_dbm = zeros(size(raw_counts_db));

ddm_power_dbm(:) = interp1(ddm_counts_db_ch,ddm_power_dbm_ch,raw_counts_db(:),'spline');
ddm_power_dbm = ddm_power_dbm+std_dev_db_ch-binning_thres_db_ch+cable_loss_db_ch;

ddm_power_watts = db2pow(ddm_power_dbm-30);         % convert to watts